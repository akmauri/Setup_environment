...
1. **task-2-5.13**: Support connecting LinkedIn Company Pages [medium]
2. **task-2-5.14**: Store LinkedIn account with encrypted tokens and account info [medium]
3. **task-2-5.15**: Update token refresh service to support LinkedIn tokens [low]
4. **task-2-5.21**: Create function to validate LinkedIn token using LinkedIn API [medium]
...
2. **For Each Task (in order, respecting dependencies):**
   ...
   g. **Commit**: Commit with message `[dev-oauth-3] [task-2-5.13] Description`
...
Begin with your first task: **task-2-5.13** - Support connecting LinkedIn Company Pages
