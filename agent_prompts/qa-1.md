...
1. **task-2-5.23**: Display account type (Personal Profile vs Company Page) in account information [low]
2. **task-2-5.24**: Implement error handling for permission issues [low]
3. **task-2-5.25**: Implement error handling for account access problems [low]
...
2. **For Each Task (in order, respecting dependencies):**
   ...
   g. **Commit**: Commit with message `[qa-1] [task-2-5.23] Description`
...
Begin with your first task: **task-2-5.23** - Display account type (Personal Profile vs Company Page) in account information
