/**
 * LinkedIn OAuth Routes
 *
 * Handles LinkedIn account connection via OAuth (supports personal profiles and company pages)
 */

import { Router, Response } from 'express';
import { verifyToken, AuthenticatedRequest } from '../middleware/auth.middleware.js';
import { rateLimitMiddleware } from '../middleware/rateLimit.middleware.js';
import { linkedinOAuthConfig, getOAuthAuthUrl } from '../config/oauth.config.js';
import {
  exchangeLinkedInCode,
  getLinkedInProfile,
  getLinkedInCompanyPages,
  getLinkedInCompanyPage,
  revokeLinkedInToken,
  validateLinkedInToken,
} from '../services/linkedin.service.js';
import {
  saveSocialAccount,
  getUserSocialAccounts,
  deleteSocialAccount,
  updateSocialAccountLabel,
  countUserSocialAccounts,
  getSocialAccountById,
} from '../services/social-account.service.js';
import { checkTierLimit } from '../services/tier.service.js';

const router = Router();

// Apply rate limiting
router.use(rateLimitMiddleware);

/**
 * GET /api/v1/social/linkedin/connect
 * Initiate LinkedIn OAuth flow
 */
router.get('/connect', verifyToken, (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    const tenantId = req.user?.tenantId;

    if (!userId || !tenantId) {
      return res.status(401).json({
        error: {
          code: 'UNAUTHORIZED',
          message: 'Authentication required',
        },
      });
    }

    const state = `${tenantId}:${userId}:${Date.now()}`;
    const authUrl = getOAuthAuthUrl('linkedin', state);

    res.json({
      authUrl,
      state,
    });
  } catch (error) {
    console.error('LinkedIn OAuth initiation error:', error);
    res.status(500).json({
      error: {
        code: 'OAUTH_INIT_ERROR',
        message: error instanceof Error ? error.message : 'Failed to initiate LinkedIn OAuth',
      },
    });
  }
});

/**
 * GET /api/v1/social/linkedin/callback
 * Handle LinkedIn OAuth callback
 */
router.get('/callback', verifyToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    const tenantId = req.user?.tenantId;

    if (!userId || !tenantId) {
      return res.status(401).json({
        error: {
          code: 'UNAUTHORIZED',
          message: 'Authentication required',
        },
      });
    }

    const { code, error: oauthError, account_type, company_id } = req.query;

    if (oauthError) {
      return res.status(400).json({
        error: {
          code: 'OAUTH_ERROR',
          message: oauthError as string,
        },
      });
    }

    if (!code) {
      return res.status(400).json({
        error: {
          code: 'MISSING_CODE',
          message: 'Authorization code is required',
        },
      });
    }

    // Check tier limits
    const accountCount = await countUserSocialAccounts(tenantId, userId, 'linkedin');
    const tierCheck = await checkTierLimit(tenantId, 'linkedin', accountCount);

    if (!tierCheck.allowed) {
      return res.status(403).json({
        error: {
          code: 'TIER_LIMIT_EXCEEDED',
          message: `You have reached the maximum number of LinkedIn accounts for your tier (${tierCheck.limit}). Upgrade to connect more accounts.`,
        },
      });
    }

    // Exchange code for tokens
    const tokenResponse = await exchangeLinkedInCode(code as string);

    // Determine account type: 'personal' or 'company'
    const isCompany = account_type === 'company' && company_id;

    if (isCompany) {
      // Connect company page
      const companyPage = await getLinkedInCompanyPage(
        tokenResponse.access_token,
        company_id as string
      );

      // Calculate token expiry (60 days for long-lived)
      const tokenExpiresAt = new Date();
      tokenExpiresAt.setSeconds(
        tokenExpiresAt.getSeconds() + (tokenResponse.expires_in || 5184000)
      );

      // Save company page account
      const account = await saveSocialAccount(tenantId, {
        userId,
        platform: 'linkedin',
        platformUserId: companyPage.id,
        username: companyPage.vanityName || companyPage.name,
        displayName: companyPage.name,
        accessToken: tokenResponse.access_token,
        refreshToken: tokenResponse.refresh_token,
        tokenExpiresAt,
        scopes: linkedinOAuthConfig.scopes,
        metadata: {
          accountType: 'company',
          companyId: companyPage.id,
          companyName: companyPage.name,
          vanityName: companyPage.vanityName,
          logoUrl: companyPage.logoUrl,
          followerCount: companyPage.followerCount,
        },
      });

      res.json({
        success: true,
        account: {
          id: account.id,
          platform: account.platform,
          username: account.username,
          display_name: account.display_name,
          label: account.label,
          metadata: account.metadata,
        },
      });
    } else {
      // Connect personal profile
      const profile = await getLinkedInProfile(tokenResponse.access_token);

      // Calculate token expiry
      const tokenExpiresAt = new Date();
      tokenExpiresAt.setSeconds(
        tokenExpiresAt.getSeconds() + (tokenResponse.expires_in || 5184000)
      );

      // Save personal profile account
      const account = await saveSocialAccount(tenantId, {
        userId,
        platform: 'linkedin',
        platformUserId: profile.sub,
        username: profile.name || profile.email || profile.sub,
        displayName: profile.name,
        accessToken: tokenResponse.access_token,
        refreshToken: tokenResponse.refresh_token,
        tokenExpiresAt,
        scopes: linkedinOAuthConfig.scopes,
        metadata: {
          accountType: 'personal',
          userId: profile.sub,
          name: profile.name,
          email: profile.email,
          picture: profile.picture,
        },
      });

      res.json({
        success: true,
        account: {
          id: account.id,
          platform: account.platform,
          username: account.username,
          display_name: account.display_name,
          label: account.label,
          metadata: account.metadata,
        },
      });
    }
  } catch (error) {
    console.error('LinkedIn OAuth callback error:', error);
    res.status(500).json({
      error: {
        code: 'OAUTH_CALLBACK_ERROR',
        message: error instanceof Error ? error.message : 'LinkedIn OAuth callback failed',
      },
    });
  }
});

/**
 * GET /api/v1/social/linkedin/companies
 * Get LinkedIn company pages for user (where user has admin access)
 */
router.get('/companies', verifyToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { access_token } = req.query;

    if (!access_token) {
      return res.status(400).json({
        error: {
          code: 'MISSING_TOKEN',
          message: 'Access token is required',
        },
      });
    }

    const companyPages = await getLinkedInCompanyPages(access_token as string);

    res.json({
      success: true,
      companies: companyPages.map((page) => ({
        id: page.id,
        name: page.name,
        vanityName: page.vanityName,
        logoUrl: page.logoUrl,
        followerCount: page.followerCount,
      })),
    });
  } catch (error) {
    console.error('Get LinkedIn company pages error:', error);
    res.status(500).json({
      error: {
        code: 'GET_COMPANIES_ERROR',
        message: error instanceof Error ? error.message : 'Failed to get LinkedIn company pages',
      },
    });
  }
});

/**
 * GET /api/v1/social/linkedin/accounts
 * List connected LinkedIn accounts
 */
router.get('/accounts', verifyToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.userId;
    const tenantId = req.user?.tenantId;

    if (!userId || !tenantId) {
      return res.status(401).json({
        error: {
          code: 'UNAUTHORIZED',
          message: 'Authentication required',
        },
      });
    }

    const accounts = await getUserSocialAccounts(tenantId, userId, 'linkedin');

    res.json({
      success: true,
      accounts: accounts.map((account) => ({
        id: account.id,
        platform: account.platform,
        platform_user_id: account.platform_user_id,
        username: account.username,
        display_name: account.display_name,
        label: account.label,
        metadata: account.metadata,
        token_expires_at: account.token_expires_at,
        is_active: account.is_active,
        created_at: account.created_at,
      })),
    });
  } catch (error) {
    console.error('List LinkedIn accounts error:', error);
    res.status(500).json({
      error: {
        code: 'LIST_ACCOUNTS_ERROR',
        message: error instanceof Error ? error.message : 'Failed to list LinkedIn accounts',
      },
    });
  }
});

/**
 * DELETE /api/v1/social/linkedin/accounts/:accountId
 * Disconnect LinkedIn account
 */
router.delete(
  '/accounts/:accountId',
  verifyToken,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.userId;
      const tenantId = req.user?.tenantId;
      const { accountId } = req.params;

      if (!userId || !tenantId) {
        return res.status(401).json({
          error: {
            code: 'UNAUTHORIZED',
            message: 'Authentication required',
          },
        });
      }

      // Get account to verify ownership and get tokens for revocation
      const account = await getSocialAccountById(tenantId, accountId);

      if (!account || account.user_id !== userId) {
        return res.status(404).json({
          error: {
            code: 'ACCOUNT_NOT_FOUND',
            message: 'LinkedIn account not found',
          },
        });
      }

      // Revoke tokens
      try {
        if (account.decryptedAccessToken) {
          await revokeLinkedInToken(account.decryptedAccessToken);
        }
      } catch (error) {
        console.error('Failed to revoke token (continuing with deletion):', error);
        // Continue with deletion even if revocation fails
      }

      // Delete account
      await deleteSocialAccount(tenantId, accountId);

      res.json({
        success: true,
        message: 'LinkedIn account disconnected successfully',
      });
    } catch (error) {
      console.error('Disconnect LinkedIn account error:', error);
      res.status(500).json({
        error: {
          code: 'DISCONNECT_ERROR',
          message: error instanceof Error ? error.message : 'Failed to disconnect LinkedIn account',
        },
      });
    }
  }
);

/**
 * PUT /api/v1/social/linkedin/accounts/:accountId/label
 * Update account label
 */
router.put(
  '/accounts/:accountId/label',
  verifyToken,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.userId;
      const tenantId = req.user?.tenantId;
      const { accountId } = req.params;
      const { label } = req.body;

      if (!userId || !tenantId) {
        return res.status(401).json({
          error: {
            code: 'UNAUTHORIZED',
            message: 'Authentication required',
          },
        });
      }

      if (typeof label !== 'string' || label.length > 255) {
        return res.status(400).json({
          error: {
            code: 'INVALID_LABEL',
            message: 'Label must be a string with max 255 characters',
          },
        });
      }

      // Verify account ownership
      const account = await getSocialAccountById(tenantId, accountId);
      if (!account || account.user_id !== userId) {
        return res.status(404).json({
          error: {
            code: 'ACCOUNT_NOT_FOUND',
            message: 'LinkedIn account not found',
          },
        });
      }

      const updated = await updateSocialAccountLabel(tenantId, accountId, label);

      res.json({
        success: true,
        account: {
          id: updated?.id,
          label: updated?.label,
        },
      });
    } catch (error) {
      console.error('Update account label error:', error);
      res.status(500).json({
        error: {
          code: 'UPDATE_LABEL_ERROR',
          message: error instanceof Error ? error.message : 'Failed to update account label',
        },
      });
    }
  }
);

/**
 * GET /api/v1/social/linkedin/accounts/:accountId/health
 * Check token health for a specific account
 */
router.get(
  '/accounts/:accountId/health',
  verifyToken,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.userId;
      const tenantId = req.user?.tenantId;
      const { accountId } = req.params;

      if (!userId || !tenantId) {
        return res.status(401).json({
          error: {
            code: 'UNAUTHORIZED',
            message: 'Authentication required',
          },
        });
      }

      const account = await getSocialAccountById(tenantId, accountId);

      if (!account || account.user_id !== userId) {
        return res.status(404).json({
          error: {
            code: 'ACCOUNT_NOT_FOUND',
            message: 'LinkedIn account not found',
          },
        });
      }

      const isValid = await validateLinkedInToken(account.decryptedAccessToken);
      const isExpired = account.token_expires_at
        ? new Date(account.token_expires_at) < new Date()
        : false;

      res.json({
        success: true,
        health: {
          valid: isValid,
          expired: isExpired,
          expires_at: account.token_expires_at,
          needs_refresh: !isValid || isExpired,
        },
      });
    } catch (error) {
      console.error('Token health check error:', error);
      res.status(500).json({
        error: {
          code: 'HEALTH_CHECK_ERROR',
          message: error instanceof Error ? error.message : 'Failed to check token health',
        },
      });
    }
  }
);

export default router;
